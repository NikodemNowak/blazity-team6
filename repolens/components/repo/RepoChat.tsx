...
export default function RepoChat() {
  ...
  // Start a fresh thread whenever the active repository changes.
  useEffect(() => {
    setMessages([]);
  }, [repoId]);
  ...
  async function send() {
    if (!repoId || !input.trim() || streaming) return;
  ...
  if (!repo) {
    return (
      <div className="empty chat-empty">
        ...
        <div>Load a GitHub repository above to start chatting.</div>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="chat-thread">
        {messages.length === 0 && (
          <div className="empty chat-empty">
            <Icon.Chat className="empty-icon" />
