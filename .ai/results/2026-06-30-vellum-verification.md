# Vellum Verification — 2026-06-30

## Scope

Verified Markdown instructions and Vellum behavior against the current app state.
Vellum is the active product surface; `repolens/` is an earlier scaffold/test app.
...
- `vellum/lib/repo/docTooling.ts` detects language documentation profiles:
...
- `npm run build` in `vellum/` passes.
- Remote changes rebased during verification added real Drift and GitHub App
  connect flow; documentation was updated to reflect those as current Vellum
  behavior.
...
- `npm run lint` in `vellum/` still invokes deprecated `next lint` and is not a valid
